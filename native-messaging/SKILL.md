...
description: >-
  ...
  拡張側サンプル（manifest.json + chrome.runtime.sendNativeMessage）を一式生成する。
...
extension/           # 動作確認用の最小拡張サンプル
  ...
  background.js      # sendNativeMessage で host と通信
  popup.html / popup.js
```
...
詳細なプロトコル仕様・`connectNative` での常駐通信・トラブルシューティングは
...
- `extension/background.js` の `const HOST_NAME = ...`
